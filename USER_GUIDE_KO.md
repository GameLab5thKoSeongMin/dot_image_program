...
선택한 이미지는 서버로 업로드되지 않고 현재 브라우저 안에서만 처리됩니다.

화면은 네 영역으로 구성됩니다.
- 왼쪽 위: 입력 이미지
- 오른쪽 위: 변환 결과와 출력 형식
- 왼쪽 아래: 파일 입력과 변환 옵션
- 오른쪽 아래: 결과 정보, 팔레트 편집기, 다운로드

데스크톱의 변환 옵션은 두 열로 나뉩니다. 프리셋·예제·레이어 모드는 보조 도구 열에 있고, 출력 크기·픽셀 처리는 핵심 설정 열에 있습니다. 보조 도구를 펼쳐도 핵심 설정 위치는 유지됩니다. 좁은 화면에서는 핵심 설정이 먼저 보이는 단일 열로 바뀝니다.
...
## 14. 최종 검증 상태
- 2026-07-24 최종 브라우저 테스트 결과: `111 / 111 cases passed.`
- 브라우저 콘솔 오류: 0건
