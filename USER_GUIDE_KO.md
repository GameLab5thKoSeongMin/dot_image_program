...
이 경고는 변환을 막는 오류가 아닙니다. 다만 큰 출력은 시간이 걸릴 수 있으므로 자동 반복 변환을 피하고 `미리보기 갱신` 버튼으로 명시적으로 변환합니다.
